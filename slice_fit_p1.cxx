...
void slice_fitTMP(const std::string& rsl){
    ...
    string output_path = "/Users/shuaixiangzhang/Work/current/FNAL_Work2024/rsl_analyses/v4_analysis/results/fit_Develop_slice10cm/pmt1/";
...
//    TH2F* inputTH2F = (TH2F*)inputFile->Get("cathode8XA");
    ...
    for(int i=0; i<num; ++i){
        adjustXAxisBinWidth(hists[i], 1000.0);//now range is [0, 200] in units of 100.


        if(80<=distances[i] && distances[i]<200){//hist has 100 bins
            combineBins(hists[i]);
            combineBins(hists[i]);
            combineBins(hists[i]);
        }
        if(50<=distances[i] && distances[i]<80){//hist has 50 bins
            combineBins(hists[i]);
            combineBins(hists[i]);
            combineBins(hists[i]);
            combineBins(hists[i]);
        ...
        if(distances[i] < 50){//hist has 25 bins
            combineBins(hists[i]);
            combineBins(hists[i]);
            combineBins(hists[i]);
            combineBins(hists[i]);
        ...
        vars.widthIni = 1.0;//Landau width---
        vars.widthMin = 0.001;
        vars.widthMax = 20.0;
