...
void slice_fitTMP(const std::string& rsl){
    ...
    for(int i=0; i<num; ++i){
        ...
        if(80 <= distances[i] && distances[i]<200){//hist has 200 bins; initial 800
            combineBins(hists[i]);
            combineBins(hists[i]);
            combineBins(hists[i]);
        }
        if(50 <= distances[i] && distances[i]<80){//hist has 200 bins; initial 800
            combineBins(hists[i]);
            combineBins(hists[i]);
            combineBins(hists[i]);
            combineBins(hists[i]);
        }
        if(0 <= distances[i] && distances[i]<50){//hist has 200 bins; initial 800
            combineBins(hists[i]);
            combineBins(hists[i]);
            combineBins(hists[i]);
            combineBins(hists[i]);
